<html>
    <head>
    	<meta charset="UTF8">
        <title>Villa Hípica App</title>
        <link rel="stylesheet" href="style.css" type="text/css" media="screen"/>
        <script type="text/javascript" src="gestao/js/jquery-2.1.4.min.js"></script>
        <style>
			body{
				background:#333;
				font-family:Arial;
    ...
    <body>	
		...
		<div id="msg_slideshow" class="msg_slideshow" >
			...
			<div id="msg_controls" class="msg_controls" style="display:none"><!-- right has to animate to 15px, default -110px -->
		...
		<?php 
		include 'gestao/conexao.php';
		$result = $conect->query('SELECT * FROM config WHERE id=1');
		$tempoSlide = '';
		while($row = mysqli_fetch_array($result)) {
			$tempoSlide = $row['tempo_foto_slide'];
		}
		?>
        <!-- The JavaScript -->
        <script type="text/javascript">
			...
			function atualizarFotos(){
				var tempoAtualizar= <?php echo (int)($tempoSlide*1000)?>;
				var tamanho = '';
				$.ajax({
			    	method: "POST",
			...
			function slide(){
	            $(function() {
					...
					*/
					var interval			= <?php echo (int)($tempoSlide*1000)?>;
					var playtime;
					var current 			= 0;
					var current_thumb 		= 0;
					var nmb_thumb_wrappers	= $('#msg_thumbs .msg_thumb_wrapper').length;
					var nmb_images_wrapper  = $('.msg_thumb_wrapper a').length;
					/**
					* start the slideshow
					*/
					play();
